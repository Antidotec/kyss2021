...
inline int possibly_expand_bam_data(bam1_t *b, size_t bytes)
{
    ...
    if (new_len > INT32_MAX || new_len < bytes)
    { // Too big or overflow
        errno = ENOMEM;
        return -1;
    }
...
int bam_tag2cigar(bam1_t *b, int recal_bin, int give_warning) // return 0 if CIGAR is untouched; 1 if CIGAR is updated with CG
{
    ...
    if (bam_cigar_op(cigar0[0]) != BAM_CSOFT_CLIP || bam_cigar_oplen(cigar0[0]) != c->l_qseq)
        return 0;
    ...
    if (possibly_expand_bam_data(b, n_cigar4 - fake_bytes) < 0)
        return -1;
...
int check_header(const uint8_t *header)
{
    if (header[0] != 31 || header[1] != 139 || header[2] != 8)
        return -2;
    return ((header[3] & 4) != 0 && unpackInt16((uint8_t *)&header[10]) == 6 && header[12] == 'B' && header[13] == 'C' && unpackInt16((uint8_t *)&header[14]) == 2) ? 0 : -1;
}
...
int read_bam(struct bam_block *fq, bam1_t *b, int is_be)
{
    ...
    if (Rabbit_bgzf_read(fq, x, 32) != 32)
        return -3; //读取32个字节
    if (is_be)
    {
        for (i = 0; i < 8; ++i)
    ...
    if (Rabbit_bgzf_read(fq, b->data, c->l_qname) != c->l_qname)
        return -4;
    if (b->data[c->l_qname - 1] != '\0')
    { // Try to fix missing NUL termination
...
BamBlock::BamBlock(BamBlockConfig *config)
{
    ...
    for (int i = this->read_bg; i < this->read_ed; i++)
        this->read[i] = i;
}
...
pair<bam_block *, int> BamBlock::getCompressdata()
{
    ...
    while (compress_ed == compress_bg)
    {
        ...
        if (config->complete)
            return pair<bam_block *, int>(NULL, -1);
        mtx_compress.lock();
    }
    int num = compress_bg;
